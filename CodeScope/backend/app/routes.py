...
def _analyze_with_extras(
    ...
    result['ai_explanation'] = (
        get_ai_explanation(result, code, language)
        if include_ai_explanations
        else _analyzer_only_explanation(result)
    )
    ...
    result['function_explanations'] = (
        ...
        if include_ai_explanations
        else _analyzer_function_explanations(local_analyzer.last_func_complexity_details)
    )
...
def _analyzer_only_explanation(result):
    return {
        'available': False,
        'why_this_complexity': result.get('time_complexity_reason', ''),
        'real_world_analogy': '',
        'performance_impact': '',
        'top_optimization': '',
        'detected_time_complexity': result.get('time_complexity'),
        'detected_space_complexity': result.get('space_complexity'),
        'detected_reason': result.get('time_complexity_reason', ''),
        'memory_model': result.get('memory_allocation_analysis') or {},
        'source': 'analyzer',
    }


def _analyzer_function_explanations(function_details):
    detail_items = (
        function_details.values()
        if isinstance(function_details, dict)
        else function_details or []
    )
    explanations = []
...
@main.route('/api/optimize/code', methods=['POST'])
def optimize_code():
    try:
        data = request.get_json()
        if not data or 'code' not in data:
