...
class _EditDutyScreenState extends State<EditDutyScreen> {
  ...
  void initState() {
    super.initState();
    _selectedRouteId = widget.duty['routeId']?.toString();
    _selectedDriverId = widget.duty['driver']?['driverId']?.toString();
    _selectedBusId = widget.duty['bus']?['busId']?.toString();
    _startTime = widget.duty['scheduledStartTime']?.toString();
    _selectedDate = _parseDate(widget.duty['scheduledDate']);
    _loadOptions();
  }
  ...
  Future<void> _save() async {
    ...
    final busId = _selectedBusId ?? _bestBusId();
    if (busId == null) {
      _showSnack('No bus available for this duty', isError: true);
      return;
    }

    setState(() => _isLoading = true);
    try {
      final res = await ApiService.put('/admin/duties/${widget.duty['dutyId']}', {
      ...
      if (res['success'] == true) {
        ...
        _showSnack(res['message'] ?? 'Failed to update duty', isError: true);
      }
    } catch (_) {
      if (!mounted) return;
      setState(() => _isLoading = false);
  ...
  Widget _routeDropdown() {
    return DropdownButtonFormField<String>(
      initialValue: _containsId(_routes, _selectedRouteId) ? _selectedRouteId : null,
      isExpanded: true,
  ...
  Widget _driverDropdown() {
    return DropdownButtonFormField<String>(
      initialValue: _containsId(_drivers, _selectedDriverId) ? _selectedDriverId : null,
      isExpanded: true,
      decoration: _inputDecoration(),
  ...
  bool _containsId(List<dynamic> items, String? id) {
    ...
    return items.whereType<Map>().any((item) => _idOf(item) == id);
  }
